...
from .target_aimle import BaseTargetDistribution, TargetDistribution
    ...
    def wrapper(theta: Tensor, *args):
        class WrappedFunc(torch.autograd.Function):
            ...
            def forward(ctx, theta: Tensor, *args):
                ...
                perturbed_theta_shape = [batch_size, nb_total_samples] + list(
                    instance_shape
                )
                ...
                if noise_distribution is None:
                    noise = torch.zeros(
                        size=torch.Size(perturbed_theta_shape), device=theta.device
                    )
                else:
                    noise = noise_distribution.sample(
                        shape=torch.Size(perturbed_theta_shape)
                    ).to(theta.device)
                ...
                # [BATCH_SIZE, N_TOTAL_SAMPLES, ...]
                perturbed_theta_3d = (
                    theta.view(batch_size, 1, -1)
                    .repeat(1, nb_total_samples, 1)
                    .view(perturbed_theta_shape)
                )
                perturbed_theta_3d = perturbed_theta_3d + eps

                # [BATCH_SIZE * N_TOTAL_SAMPLES, ...]
                perturbed_theta_2d = perturbed_theta_3d.view(
                    [-1] + perturbed_theta_shape[2:]
                )
                ...
                # z = MAP(θ + ε)
                # [BATCH_SIZE * N_TOTAL_SAMPLES, ...]
                z_2d, _ = function(perturbed_theta_2d)
            ...
            @staticmethod
            def backward(ctx, dy):
                # for debugging, enable the following two lines
                # import pydevd
                ...
                # θ'_R = θ - λ dy
                target_theta_r_2d = target_distribution.params(
                    theta_2d, dy, _is_minimization=_is_minimization
                )
                # θ'_L = θ + λ dy -- if symmetric_perturbation is False, then this reduces to θ'_L = θ
                target_theta_l_2d = target_distribution.params(
                    theta_2d,
                    -dy if symmetric_perturbation else None,
                    _is_minimization=_is_minimization,
                )
                ...
                with torch.inference_mode():
                    # z'_R = MAP(θ'_R + ε)
                    z_r_2d, _ = function(perturbed_target_theta_r_2d)

                    # z'_L = MAP(θ'_L + ε)
                    z_l_2d, _ = function(perturbed_target_theta_l_2d)
                ...
                if nb_marginal_samples > 1:
                    ...
                    z_l_4d = z_l_3d.view(
                        [batch_size, nb_samples, nb_marginal_samples]
                        + list(instance_shape)
                    )
                    z_r_4d = z_r_3d.view(
                        [batch_size, nb_samples, nb_marginal_samples]
