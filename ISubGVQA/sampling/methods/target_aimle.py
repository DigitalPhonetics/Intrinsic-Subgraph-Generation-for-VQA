...
class BaseTargetDistribution(ABC):
    ...
    @abstractmethod
    def params(
        self, theta: Tensor, dy: Optional[Tensor], _is_minimization: bool = False
    ) -> Tensor:
        raise NotImplementedError

    @abstractmethod
    def process(self, theta: Tensor, dy: Tensor, gradient: Tensor) -> Tensor:
        return gradient
...
class TargetDistribution(BaseTargetDistribution):
    ...
    def process(self, theta: Tensor, dy: Tensor, gradient_3d: Tensor) -> Tensor:
        scaling_factor = max(self.beta, self.eps)
        res = (
            (gradient_3d / scaling_factor)
            if self.do_gradient_scaling is True
            else gradient_3d
        )
        return res
...
class AdaptiveTargetDistribution(BaseTargetDistribution):
    ...
    def _perturbation_magnitude(self, theta: Tensor, dy: Optional[Tensor]):
        norm_dy = torch.linalg.norm(dy).item() if dy is not None else 1.0
        return (
            0.0 if norm_dy <= 0.0 else self.beta * (torch.linalg.norm(theta) / norm_dy)
        )
    ...
    def process(self, theta: Tensor, dy: Tensor, gradient_3d: Tensor) -> Tensor:
        ...
        self.grad_norm = self.grad_norm_decay_rate * self.grad_norm + (
            1.0 - self.grad_norm_decay_rate
        ) * (grad_nnz / nb_gradients)

        # If the gradient norm is lower than 1, we increase beta; otherwise, we decrease beta.
        beta_update_ = (
            1.0 if self.grad_norm.item() < self.target_norm else -1.0
        ) * self.beta_update_step
        beta_update = (
            self.beta_update_momentum * self.previous_beta_update
        ) + beta_update_
