...
from .simple import Layer
from .tensor_utils import self_defined_softmax

LARGE_NUMBER = 1.0e10


def logsigmoid(x):
    return -F.softplus(-x) + 1.0e-7
...
class EdgeSIMPLEBatched(nn.Module):
    ...
    def forward(self, scores, train=True):
        ...
        if self.policy == "global_directed":
            ...
            flat_scores = scores.permute((0, 3, 1, 2)).reshape(
                bsz * ensemble, target_size
            )
        ...
        if self.logits_activation == "None" or self.logits_activation is None:
            ...
            flat_scores = torch.vmap(self_defined_softmax, in_dims=0, out_dims=0)(
                flat_scores, masks
            )
        ...
        if self.policy == "global_directed":
            new_mask = samples.reshape(
                times_sampled, bsz, ensemble, Nmax, Nmax
            ).permute((0, 1, 3, 4, 2))
            new_marginals = marginals.reshape(bsz, ensemble, Nmax, Nmax).permute(
                (0, 2, 3, 1)
            )
        elif self.policy == "global_undirected":
            samples = samples.reshape(times_sampled, bsz, ensemble, -1).permute(
                (0, 1, 3, 2)
            )
            ...
            new_mask = samples.reshape(times_sampled, bsz, ensemble, Nmax).permute(
                (0, 1, 3, 2)
            )
